...
object PongJLine extends App {
  ...
  def draw(): Unit = {
    terminal.writer().print("\u001b[H\u001b[2J") // Clear screen
    for (y <- 0 until height) {
      for (x <- 0 until width) {
        if (x == 0 && y >= leftPaddleY && y < leftPaddleY + paddleHeight) {
          terminal.writer().print(s"${BLUE}|${RESET}")
        } else if (
          x == width - 1 && y >= rightPaddleY && y < rightPaddleY + paddleHeight
        ) {
          terminal.writer().print(s"${RED}|${RESET}")
        } else if (x == ballX && y == ballY) {
          terminal.writer().print(s"${GREEN}O${RESET}")
        } else {
    ...
    terminal.writer().println(s"Score: Left $leftScore - Right $rightScore")
    terminal
      .writer()
  ...
  def update(): Unit = {
    ...
    if (
      ballX == 1 && ballY >= leftPaddleY && ballY < leftPaddleY + paddleHeight
    ) {
      ballDirX = -ballDirX
    } else if (
  ...
  def handleInput(input: Char): Unit = input.toLower match {
